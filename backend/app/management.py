...
class AssumptionsConfig(StrictModel):
    ...
    assumptions: Annotated[list[Assumption], Field(min_length=15)]
...
def _check_config(config, assumptions, decision):
    ...
    if len(ids) != len(entries) or not {f"A{i:02d}" for i in range(1, 16)} <= ids:
...
FINANCE_FIELDS = ("c0_mrub", "opex_mrub_per_year", "anchor_cash_mrub_per_year", "commercial_cash_mrub_per_year", "cash_mrub_per_year")
...
def finance_csv(bundle):
    ...
    columns = ["release_id", "strategy_id", "lot_id", "mode_id", *FINANCE_FIELDS, "net_operating_balance", "lot_funding_gap", "portfolio_funding_gap", "sum_lot_funding_gaps"]
