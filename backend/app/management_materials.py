...
def render_documents(bundle):
    ...
    boundary = ("C0 — млн руб. при запуске; OPEX, anchor, commercial, CASH и gaps — млн руб./год устойчивой эксплуатации. "
                "VPUB — отдельная синтетическая общественная шкала в млн руб./год. KCASH — отношение сумм CASH/OPEX, не прибыльность.")
    ...
    assumption_text = table(["Допущение", "Статус / значение", "Ограничение или уточнение"],
        [[a['id']+' '+a['topic'],a['status']+': '+str(a.get('value') if a.get('value') is not None else 'неизвестно'),a.get('limitation',a.get('reason',''))+' '+a.get('resolve_by','')] for a in bundle['assumptions']['assumptions']])
    method = ("Модель применима к выбору небольшой четвёрки из синтетических лотов при заданных условиях: "
              ...
              "Расшифровка t_rep и методики исходных индексов неизвестны; это не окупаемость. "
