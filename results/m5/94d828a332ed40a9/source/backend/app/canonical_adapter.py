...
ASSUMPTIONS = [{"file": "config/assumptions.json", "id": value} for value in ("A02", "A04", "A05", "A06", "A15")]
...
def evaluate_validated(request: EvaluateRequest, snapshot: CaseSnapshot) -> dict:
    ...
    bundle = {
        ...
        "scenarios": scenario_diagnostics(metrics, snapshot.config, snapshot.core, complete),
        "units": UNITS, "period": PERIOD, "assumption_refs": ASSUMPTIONS,
...
def evaluate(value, repository: CaseRepository | None = None) -> dict:
    ...
    return evaluate_validated(request, (repository or CaseRepository()).load())
...
def case_description(snapshot: CaseSnapshot) -> dict:
    return {"schema_version": SCHEMA_VERSION, "adapter_version": ADAPTER_VERSION,
            ...
            "scenarios": snapshot.config["scenarios"], "units": UNITS, "period": PERIOD, "assumption_refs": ASSUMPTIONS,
