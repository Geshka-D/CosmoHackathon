...
test('complete 10-chapter jury demo, actual shock, recovery, locks, finance, brief, Back/Reset/exit', async ({ page, request }) => {
  ...
  await page.getByRole('button', { name: 'Начать защиту · Pitch Mode' }).click()
  ...
  await controls.getByRole('button', { name: 'Reset Pitch', exact: true }).click()
  await expect(page.locator('#mission')).toBeVisible()
  await page.keyboard.press('Escape')
  await expect(page.locator('.product')).not.toHaveClass(/pitch-active/)
  await expect(page.getByRole('button', { name: 'Начать защиту · Pitch Mode' })).toBeFocused()
...
for (const width of [390, 768]) test(`responsive ${width}, reduced motion, local-only requests and Pitch exit`, async ({ page, request, baseURL }) => {
  ...
  await page.getByRole('button', { name: 'Выйти из Pitch', exact: true }).click()
  await expect(page.getByRole('button', { name: 'Начать защиту · Pitch Mode' })).toBeVisible()
