...
function FinanceTable({ rows, totals }: { rows: FinanceRow[]; totals: Money }) {
  const cells = (r: Omit<FinanceRow, 'lot_id' | 'mode_id'>) => <><td>{format(r.c0_mrub)}</td><td>{format(r.opex_mrub_per_year)}</td><td>{format(r.anchor_cash_mrub_per_year)}</td><td>{format(r.commercial_cash_mrub_per_year)}</td><td>{format(r.cash_mrub_per_year)}</td><td>{format(r.net_operating_balance)}</td><td>{format(r.lot_funding_gap)}</td></>
  return <div className="table-scroll" role="region" aria-label="Финансирование сохранённого портфеля" tabIndex={0}><table><thead><tr><th>Сервис</th><th>C0<small>млн руб.</small></th><th>OPEX</th><th>Anchor</th><th>Commercial</th><th>CASH</th><th>Баланс</th><th>Адресный gap</th></tr></thead><tbody>{rows.map(r => <tr key={r.lot_id}><th scope="row">{r.lot_id} {r.mode_id}</th>{cells(r)}</tr>)}<tr className="implementation-total"><th scope="row">Итого</th>{cells({ ...totals, lot_funding_gap: totals.sum_lot_funding_gaps })}</tr></tbody></table></div>
}
