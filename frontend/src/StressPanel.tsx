  ...
  return <section id="stress" className="panel stress" aria-labelledby="stress-title" aria-busy={pending}>
    ...
    {!result ? <p role="status">{pending ? 'Пересчитываем оба сценария для текущего ввода…' : 'Результат скрыт до успешного пересчёта. Исправьте ошибку выше.'}</p> : <>
      ...
      {complete && !stressOk && <div className="stress-actions">
        ...
        <p>Ниже — варианты текущего шортлиста, которые сервер отметил допустимыми в STRESS. Заявленное правило метода выбирает <b>лучшего по score</b>; отдельного механизма «минимально изменить состав» в принятой модели нет, поэтому такой вариант не предлагается как рассчитанный. Колонка «отличие состава» — точное сравнение наборов лотов и режимов, а не оценка близости.</p>
        {candidates.length === 0
